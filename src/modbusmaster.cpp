...
void ModbusMaster::initComboBox_Config()
{
    /* 更新下拉列表 */
...
/* 刷新按钮点击 槽函数 */
void ModbusMaster::on_btnRefresh_clicked()
{
...
/* 打开串口 槽函数*/
void ModbusMaster::on_btnOpenPort_clicked(bool checked)
{
...
/* 串口错误信息处理 */
void ModbusMaster::slots_errorHandler(QSerialPort::SerialPortError error)
{
    switch(error){
 ...
 */
void ModbusMaster::setLineEditInputType()
{
    ...
    ui->txt03SlaveAddr->setAttribute(Qt::WA_Hover,true); // 安装事件过滤器
    ui->txt03SlaveAddr->installEventFilter(this);

    ui->txt03RegAddr->setValidator(reg);
    ui->txt03RegAddr->setAttribute(Qt::WA_Hover,true);
    ui->txt03RegAddr->installEventFilter(this);

    ui->txt03RegNum->setValidator(reg);
    ui->txt03RegNum->setAttribute(Qt::WA_Hover,true);
    ui->txt03RegNum->installEventFilter(this);


    ui->txt04SlaveAddr->setValidator(reg);// 设定正则表达式
    ui->txt04SlaveAddr->setAttribute(Qt::WA_Hover,true);
    ui->txt04SlaveAddr->installEventFilter(this);

    ui->txt04RegAddr->setValidator(reg);
    ui->txt04RegAddr->setAttribute(Qt::WA_Hover,true);
    ui->txt04RegAddr->installEventFilter(this);

    ui->txt04RegNum->setValidator(reg);
    ui->txt04RegNum->setAttribute(Qt::WA_Hover,true);
    ui->txt04RegNum->installEventFilter(this);

    ui->txt06SlaveAddr->setValidator(reg);// 设定正则表达式
    ui->txt06SlaveAddr->setAttribute(Qt::WA_Hover,true);
    ui->txt06SlaveAddr->installEventFilter(this);

    ui->txt06RegAddr->setValidator(reg);
    ui->txt06RegAddr->setAttribute(Qt::WA_Hover,true);
    ui->txt06RegAddr->installEventFilter(this);

    ui->txt06Value->setValidator(reg);
    ui->txt06Value->setAttribute(Qt::WA_Hover,true);
    ui->txt06Value->installEventFilter(this);

    ui->txt10SlaveAddr->setValidator(reg);
    ui->txt10SlaveAddr->setAttribute(Qt::WA_Hover,true);
    ui->txt10SlaveAddr->installEventFilter(this);

    ui->txt10RegAddr->setValidator(reg);
    ui->txt10RegAddr->setAttribute(Qt::WA_Hover,true);
    ui->txt10RegAddr->installEventFilter(this);

    ui->txt10RegNum->setValidator(reg);
    ui->txt10RegNum->setAttribute(Qt::WA_Hover,true);
    ui->txt10RegNum->installEventFilter(this);

    ui->txt10Value->setValidator(reg);
    ui->txt10Value->setAttribute(Qt::WA_Hover,true);
    ui->txt10Value->installEventFilter(this);


    ui->txtOtherValue->setValidator(reg);// 设定正则表达式
    ui->txtOtherValue->setAttribute(Qt::WA_Hover,true);
    ui->txtOtherValue->installEventFilter(this);

    ui->btn03Send->setAttribute(Qt::WA_Hover,true); // 安装事件过滤器
    ui->btn03Send->installEventFilter(this);

    ui->btn04Send->setAttribute(Qt::WA_Hover,true); // 安装事件过滤器
    ui->btn04Send->installEventFilter(this);

    ui->btn06Send->setAttribute(Qt::WA_Hover,true); // 安装事件过滤器
    ui->btn06Send->installEventFilter(this);

    ui->btn10Send->setAttribute(Qt::WA_Hover,true); // 安装事件过滤器
    ui->btn10Send->installEventFilter(this);

    ui->btnOtherSend->setAttribute(Qt::WA_Hover,true); // 安装事件过滤器
    ui->btnOtherSend->installEventFilter(this);
}
/**
 * @brief ModbusTool::eventFilter  事件过滤器，用于处理鼠标悬停进入事件
 * @param obj      // 触发事件的对象
 * @param event   // 触发事件类型
 * @return
 */
bool ModbusMaster::eventFilter(QObject *obj, QEvent *event)
{
    if(event->type() == QEvent::HoverEnter){ // 鼠标悬停进入
        if( obj == ui->txt03SlaveAddr ){
            ui->lblTips->setText(" 03H，从机地址 ");
        }else if( obj == ui->txt03RegAddr ){
            ui->lblTips->setText(" 03H，待读取的寄存器首地址 ");
        }else if( obj == ui->txt03RegNum ){
            ui->lblTips->setText(" 03H，待读取的寄存器数量 ");
        }else if( obj == ui->txt04SlaveAddr ){
            ui->lblTips->setText(" 04H，从机地址 ");
            return true;
        }else if( obj == ui->txt04RegAddr ){
            ui->lblTips->setText(" 04H，待读取的寄存器首地址 ");
            return true;
        }else if( obj == ui->txt04RegNum ){
            ui->lblTips->setText(" 04H，待读取的寄存器数量 ");
            return true;
        }else if( obj == ui->txt06SlaveAddr ){
            ui->lblTips->setText(" 06H，从机地址 ");
            return true;
        }else if( obj == ui->txt06RegAddr ){
            ui->lblTips->setText(" 06H，待写入的寄存器地址 ");
            return true;
        }else if( obj == ui->txt06Value ){
            ui->lblTips->setText(" 06H，待写入的寄存器值 ");
            return true;
        }else if( obj == ui->txt10SlaveAddr ){
            ui->lblTips->setText(" 10H，从机地址 ");
            return true;
        }else if( obj == ui->txt10RegAddr ){
            ui->lblTips->setText(" 10H，待写入的寄存器首地址 ");
            return true;
        }else if( obj == ui->txt10RegNum ){
            ui->lblTips->setText(" 10H，待写入的寄存器数量 ");
            return true;
        }else if( obj == ui->txt10Value ){
            ui->lblTips->setText(" 10H，待写入的寄存器值，可以用空格隔开的十六进制字符 ");
            return true;
        }else if( obj == ui->txtOtherValue ){
            ui->lblTips->setText(" 其他指令，无需输入起始符(:)、校验码、结束符(\\r\\n),只需输入十六进制字符或者空格");
            return true;
        }else if( obj == ui->btn03Send ){
            ui->lblTips->setText(" 03H功能码，读取多个Holding Registers");
            return true;
        }else if( obj == ui->btn04Send ){
            ui->lblTips->setText(" 04H功能码，读取多个Input Registers");
            return true;
        }else if( obj == ui->btn06Send ){
            ui->lblTips->setText(" 06H功能码，写入单个Holding Register");
            return true;
        }else if( obj == ui->btn10Send ){
            ui->lblTips->setText(" 10H功能码，写入多个Holding Registers");
            return true;
        }else if( obj == ui->btnOtherSend ){
            ui->lblTips->setText(" 其他指令，可用于发送其他的Modbus功能码");
            return true;
        }
    }
 ...
 * @param arg1 输入的字符
 */
void ModbusMaster::on_txt03SlaveAddr_textChanged(const QString &arg1)
{
    QString tmp = arg1;
    if(arg1.size()>2 ){
 ...
 * @param arg1 输入的字符
 */
void ModbusMaster::on_txt03RegAddr_textChanged(const QString &arg1)
{
    QString tmp = arg1;
    if(arg1.size()>4 ){
        QString last = tmp.right(1);
...
void ModbusMaster::on_txt03RegNum_textChanged(const QString &arg1)
{
    QString tmp = arg1;
    if(arg1.size()>4 ){
        tmp.chop(1);
 ...
 * @param arg1 输入的字符
 */
void ModbusMaster::on_txt04SlaveAddr_textChanged(const QString &arg1)
{
    QString tmp = arg1;
    if(arg1.size()>2 ){
        QString last = tmp.right(1);
 ...
 * @param arg1 输入的字符
 */
void ModbusMaster::on_txt04RegAddr_textChanged(const QString &arg1)
{
    QString tmp = arg1;
    if(arg1.size()>4 ){
        QString last = tmp.right(1);
...
void ModbusMaster::on_txt04RegNum_textChanged(const QString &arg1)
{
    QString tmp = arg1;
    if(arg1.size()>4 ){
        tmp.chop(1);
 ...
 * @param arg1 输入的字符
 */
void ModbusMaster::on_txt06SlaveAddr_textChanged(const QString &arg1)
{
    QString tmp = arg1;
    if(arg1.size()>2 ){
        QString last = tmp.right(1);
 ...
 * @param arg1 输入的字符
 */
void ModbusMaster::on_txt06RegAddr_textChanged(const QString &arg1)
{
    QString tmp = arg1;
    if(arg1.size()>4 ){
        QString last = tmp.right(1);
...
void ModbusMaster::on_txt06Value_textChanged(const QString &arg1)
{
    QString tmp = arg1;
    if(arg1.size()>4 ){
        tmp.chop(1);
 ...
 * @param arg1 输入的字符
 */
void ModbusMaster::on_txt10SlaveAddr_textChanged(const QString &arg1)
{
    QString tmp = arg1;
    if(arg1.size()>2 ){
        QString last = tmp.right(1);
 ...
 * @param arg1 输入的字符
 */
void ModbusMaster::on_txt10RegAddr_textChanged(const QString &arg1)
{
    QString tmp = arg1;
    if(arg1.size()>4 ){
        QString last = tmp.right(1);
...
void ModbusMaster::on_txt10RegNum_textChanged(const QString &arg1)
{
    QString tmp = arg1;
    if(arg1.size()>4 ){
        QString last = tmp.right(1);
 ...
 */
void ModbusMaster::on_btnOtherSend_clicked()
{
 ...
 */
void ModbusMaster::on_btn03Send_clicked()
{
 ...
 */
void ModbusMaster::on_btn04Send_clicked()
{
    if(!currentPort->isOpen()) return ;
 ...
 */
void ModbusMaster::on_btn06Send_clicked()
{
    if(!currentPort->isOpen()) return ;
 ...
 */
void ModbusMaster::on_btn10Send_clicked()
{
    if(!currentPort->isOpen()) return ;
 ...
 */
void ModbusMaster::sendFrame(QByteArray txbuf)
{
    /*-------- 设置文本颜色 ----------*/
 ...
 */
void ModbusMaster::slots_RxCallback()
{
    /*-------- 设置文本颜色 ----------*/
 ...
 * @param msg
 */
void ModbusMaster::insertLogAtTime(QString msg)
{
    /* 数据显示 */
 ...
 */
void ModbusMaster::slots_waitForCRC()
{
 ...
 * @param mode
 */
void ModbusMaster::frameProtocal(QByteArray rxBuf,ProtocalMode mode)
{
...
void ModbusMaster::exceptionHandle(ExceptionCode exception )
{
 ...
 */
void ModbusMaster::on_btnExpand_clicked()
{
    QRect Multigeometry = ui->scrollArea->geometry();
    if(!isExpand){
        isExpand = true;
        ...
        this->setGeometry(geo.x(),geo.y(), geo.width()+Multigeometry.width()+6, geo.height() );
    }else{
        ui->scrollArea->setVisible(false);
        isExpand = false ;
        QRect geo = this->geometry();
 ...
 */
void ModbusMaster::initMultiwidget()
{
 ...
 */
void ModbusMaster::readMultiWidget()
{
 ...
 * @param count
 */
void ModbusMaster::addWidgetGoup(int count)
{
 ...
 * @param spb
 */
void ModbusMaster::iniSpb(QSpinBox *spb)
{
    spb->setMinimum(0);
 ...
 * @param btn
 */
void ModbusMaster::iniBtn(QPushButton *btn)
{
...
void ModbusMaster::slots_clicktoSend()
{
 ...
 * @param txt
 */
void ModbusMaster::iniTxt(QLineEdit *txt)
{
    QRegExp regExp("[a-fA-F0-9 ]*"); // 匹配十六进制字符和空格
    QRegExpValidator *reg = new QRegExpValidator(regExp,this);
 ...
 * @param checked
 */
void ModbusMaster::on_pushButton_clicked(bool checked)
{
    if(checked){
        QTimer::singleShot(10,this,SLOT(slots_singleShot()));
    }
}
void ModbusMaster::slots_singleShot()
{
...
quint16 ModbusMaster::crc16_modbus_calc(quint8 *data, quint32 length)
{
    uint16_t tmp = 0;
 ...
 * @return  lrc    LRC校验码
 */
quint8 ModbusMaster::verifyLRC(quint8 *data, quint32 length)
{
 ...
 * @param e
 */
void ModbusMaster::closeEvent(QCloseEvent *e)
{
    e = e;
    QSettings settings("ModbusMaster.ini", QSettings::IniFormat);
    settings.setIniCodec("GBK");
 ...
 * @param pos
 */
void ModbusMaster::slots_showBtnRightClickMenu(const QPoint &pos)
{
    QPoint po = pos;
    po = po;
    QPushButton *btn = qobject_cast<QPushButton*>(sender());
 ...
 */
void ModbusMaster::on_actionRename_triggered()
{
    QVariant ptrBtn;
 ...
 */
void ModbusMaster::on_btnPlus_clicked()
{
    if(isExpand){
        this->addWidgetGoup(8);
    }
}
void ModbusMaster::on_btnSubtract_clicked()
{
    if(isExpand){
        if(Count > 0){
 ...
 */
void ModbusMaster::on_actionModbusPro_triggered()
{
